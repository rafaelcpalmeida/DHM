...
public class DBMockup implements Serializable {
    ...
    private final HashMap<User, AuthSessionRI> sessions;   // User -> session
    private final HashMap<User, String> users;             // User -> passw
    private final HashMap<User, TaskGroup> taskgroups;     // User -> taskgroup
    private final HashMap<Task, ArrayList<Worker>> tasks;             // Task -> user.worker
    private final HashMap<User, Integer> userCoins;
    private final HashMap<User, ArrayList<WorkerRI>> userWorkers;
    ...
    public boolean exists(edu.ufp.inf.sd.HashFinder.client.User user) {
        for (User userAux : this.users.keySet()) {
            if (userAux.getUsername().compareTo(user.getUsername()) == 0) {
                if (this.users.get(userAux).compareTo(user.getPassword()) == 0) return true;
            }
        }
        return false;
    }
